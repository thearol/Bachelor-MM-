...
data_db_long <- read.csv("decision_bias/long_data_db.csv")
```
...
pred_ib1$Estimate <- exp(pred_ib1$Estimate)
...
H2.1.1 <- ggplot(data_ib_for_plot, aes(x = condition, y = rt)) +
  labs(x = "Condition", y = "Reaction time (seconds)", title = "Hypothesis 2.1") +
...
pred_ib2$Estimate_prop <- exp(pred_ib2$Estimate)
...
H2.2.1_lines <- ggplot(pred_ib2, aes(x = condition, y = Estimate_sec, fill = gender)) + 
  scale_color_manual(values=c("orange", "blue"), name = "Gender") + 
  stat_summary(fun.data = "mean_cl_boot", color = "black", size = 1.5, geom = "point") + 
  stat_summary(fun.y = mean, geom="line") + 
  labs(x = "Condition", y = "Reaction time (seconds)", title = "Hypothesis 2.2") + 
...
#Subtracting the pavement bias of 0.5
data_eb_r$overall_score <- data_eb_r$overall_score - 1
data_eb_r$benevolent_score <- data_eb_r$benevolent_score - 1
data_eb_r$hostile_score<- data_eb_r$hostile_score -1
...
data_eb_r$overall_score <- rescale(data_eb_r$overall_score, to = c(0, 1), from = range(0,5))
data_eb_r$benevolent_score <- rescale(data_eb_r$benevolent_score, to = c(0, 1), from = range(0,5))
data_eb_r$hostile_score <- rescale(data_eb_r$hostile_score, to = c(0, 1), from = range(0,5))


#Rerunning the model to get new estimates

#Defining a prior, mean set to 0.5 because best conceptual meaning
prior_eb1_scaled = c(prior(normal(0.75, 0.1), class = "Intercept"), 
...
#prior predictive check
model_db_conf_skill_prior <- brm(Sensitivity_roll ~ 1 + Response_abs * Gender + (1 | SubjectID), 
...
model_db_conf_skill <- brm(Sensitivity_roll ~ 1 + Response_abs * Gender + (1 | SubjectID),
  data = data_db_long,
  prior = prior_db_conf_skill,
  sample_prior=T,
  family = "gaussian", #As we had a binary outcome, we set this to "bernoulli"
