...
#Merging left data
disagree_db <- merge(disagree_db, sensitivity_scores_left, by = "SubjectID_left_unique", all.x = TRUE, all.y = TRUE )

#Merging right data
disagree_db <- merge(disagree_db, sensitivity_scores_right, by = "SubjectID_right_unique", all.x = TRUE, all.y = TRUE )

#deselect X
disagree_db <- subset(disagree_db, select = -c(X))
...
```{r}
disagree_db <- filter(disagree_db, group_id != "25_KK_27" & group_id != "25_KK_28")
```
...
disagree_db$skill_dif <- 0 #Creating column of 0
disagree_db$skill_dif <- ifelse(disagree_db$chosen_leader == "Left_lead", disagree_db$Slope_left/disagree_db$Slope_right, disagree_db$Slope_right/disagree_db$Slope_left) #calculating skill difference as a ratio between leader and follower sensitivity

```


#Plotting skill difference
```{r}
hist(disagree_db$skill_dif)
```
...
write.csv(disagree_db, file = "disagree_sensitivity_db_wide.csv")
