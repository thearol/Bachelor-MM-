...
library(pacman)
p_load(plotly, jpeg, tm, brms, tidyverse, tidybayes, ggplot2, LaplacesDemon, rethinking, tidyr, reshape2, tibble, stringr, here, brmstools, data.table, mousetrap)
...
#deselect X
disagree_db_wide <- subset(disagree_db_wide, select = -c(X))
...
disagree_db_wide <- scale_within(disagree_db_wide, variables = "Response_left", within = "SubjectID_left", center = TRUE, scale = TRUE, prefix = "z_")

#Right response scaled
disagree_db_wide <- scale_within(disagree_db_wide, variables = "Response_right", within = "SubjectID_right", center = TRUE, scale = TRUE, prefix = "z_")
...
#Scale to reduce extreme values
disagree_db_wide$conf_dif_z <-scale(disagree_db_wide$conf_dif)
```
...
```{r create ratio of confidence difference}

```


#Write CSV
```{r creating CSV, eval=F}
