...
#data
data_db <- read.csv("decision_bias/data_db.csv")[,2:41]
disagree_db <- read.csv("decision_bias/disagree_db.csv")[,2:41]
...
for (i in 1:96){ #Loop through number of trials
  if(i == 10){ #If the first 10
    for (p in unique(long_data_db$SubjectID_unique)){
          if (n == 1) {
            data_i <- filter(long_data_db, long_data_db$SubjectID_unique == p)[1:10, ]
            n = n + 1
          } else {
            data_i <- rbind(data_i, filter(long_data_db, long_data_db$SubjectID_unique == p)[1:10, ])
            n = n + 1
          } }
    ...
    ###SAVING ESTIMATES
    #Saving random effect for individual slope
    random_effects_slope <- as.data.frame(ranef(m_sensitivity))[,5]
    
    #Saving fixed effects 
    fixed_effects <- as.data.frame(fixef(m_sensitivity)[2,1]) #Making data frame of fixed effects
    
    #Make an empty dataframe
    sensitivity_scores = data.frame(matrix(vector(), 86, 128,
                    dimnames=list(c(), as.character(unique(long_data_db$SubjectID_unique)))))
    ...
    #Adding rownames
    sensitivity_scores <- cbind(Row.Names = , as.data.frame(sensitivity_scores))
h <- as.data.frame(ranef(m_sensitivity)
...
for (i in  NROW(long_data_db$SubjectID_unique)){
  if(i == 10){ 
    #Choosing temporary data
    for (p in unique(data_db$SubjectID_left)){
      if (p == )
      data_i <- filter(data_db, SubjectID_left == i)
    }
         data_i <- long_data_db[1:10,] #Choosing temporary data
