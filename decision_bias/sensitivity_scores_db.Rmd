...
# mean_centered_resp_right <- data.frame(matrix(ncol=2))#creating empty data frame
# n = 1
# 
# for (i in unique(disagree_db_wide$SubjectID_right_unique)){
#     tempdata <- subset(disagree_db_wide, SubjectID_right_unique==i)
#     mean_centered_resp[n,1] <- i
#     mean_centered_resp[n,2] <- tempdata$Response_right_abs - mean(tempdata$Response_right_abs)
#     n=n+1
# }
# 
# mean_centered_resp_left <- data.frame(matrix(ncol=2))#creating empty data frame
# n = 1
# 
# for (i in unique(disagree_db_wide$SubjectID_left_unique)){
#     tempdata <- subset(disagree_db_wide, ID==i)
#     mean_centered_resp_left[n,1] <- i
#     mean_centered_resp_left[n,2] <- tempdata$Response_left_abs - mean(tempdata$Response_left_abs)
#     n=n+1
#   }
# 
# colnames(mean_centered_resp_left) <-  c("SubjectID_left_unique", "Response_mc_left")
# colnames(mean_centered_resp_right) <-  c("SubjectID_right_unique", "Response_mc_right")
# 
# #Merging left data
# disagree_db_wide <- merge(disagree_db_wide, mean_centered_resp_left, by = "SubjectID_left_unique", all.x = TRUE, all.y = TRUE )
# 
# disagree_db_wide <- merge(disagree_db_wide, mean_centered_resp_right, by = "SubjectID_right_unique", all.x = TRUE, all.y = TRUE )
...
# 
# #Creating ratio using mean centered
# disagree_db_wide$conf_dif <- 0 #Creating column of 0
# disagree_db_wide$conf_dif <- ifelse(disagree_db_wide$chosen_leader == "Left_lead", disagree_db_wide$Response_left_abs/disagree_db_wide$Response_right_abs, disagree_db_wide$Response_right_abs/disagree_db_wide$Response_left_abs) #calculating confidence difference as a ratio between leader and follower confidence
