...
```{r creating new sensitivity score}
...
for (i in 1:96){ #Loop through number of trials
  ...
  if(i == 10){ #If the first 10
    ...
    ###SAVING ESTIMATES
    #Saving random effect for individual slope
    sensitivity_scores <- as.data.frame(ranef(m_sensitivity))[,5] + fixef(m_sensitivity)[2,1]
    
    #Adding rownames
    sensitivity_scores <- cbind(Row.Names = rownames(as.data.frame(ranef(m_sensitivity))[,5:6]), as.data.frame(sensitivity_scores))
    
    colnames(sensitivity_scores) <- c("SubjectID_unique", i)
  }
    ...
    #Saving random effect for individual slope
    sensitivity_scores_temp <- as.data.frame(ranef(m_sensitivity))[,5] + fixef(m_sensitivity)[2,1]
    
    #Cbind new results to sensitivity score
    sensitivity_scores <- cbind(sensitivity_scores, sensitivity_scores_temp)
    
    colnames(sensitivity_scores)[k+2] <- i
    
} }
    
    
  
write.csv(sensitivity_scores, file = "sensitivity_scores_rolling.csv")
```
...
#Creating a trial variable¨
 for (p in unique(long_data_db$SubjectID_unique)){
   if (n = 1) {
     data_i <- filter(long_data_db, long_data_db$SubjectID_unique == p)
     data_i <- cbind(data_i, as.data.frame(1:NROWS(data_i$SubjectID_unique)))
   } else {
     data_temp <- filter(long_data_db, long_data_db$SubjectID_unique == p)
     data_temp <- cbind(data_i, as.data.frame(1:NROWS(data_i$SubjectID_unique)))
     long_data_db_try <- rbind(data_i, data_temp)
   }
 }
