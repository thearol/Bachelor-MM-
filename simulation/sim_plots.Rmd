...
year20_dis <- read.csv("data/simple_sim_DB.csv", stringsAsFactors = F)
...
DB_long$bias <- "Bias taken from decision bias"
DB_long$Level <- rep(1:8,1)

IB_long <- as.data.frame(t(IB[1:8]))  
IB_long$bias <- "Bias taken from implicit bias"
IB_long$Level <- rep(1:8,1)

EB_long <- as.data.frame(t(EB[1:8]))  
EB_long$bias <- "Bias taken from explicit bias"
...
compare <- ggplot(compare_long, aes(x = Level, y = X1, color = bias)) + 
  ...
  scale_x_discrete(limits=c("1", "2", "3", "4", "5", "6", "7", "8")) + 
  scale_color_manual(values=c("purple", "gold", "pink", "orange")) + 
  labs(color = "Bias") + 
  scale_y_continuous(labels = dollar_format(suffix = "%", prefix = "")) + 
