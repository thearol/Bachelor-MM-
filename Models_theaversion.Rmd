...
#Model 2
#Females have higher benevolent scores 
...
hypothesis(model_eb2, "0 > genderMale:hostile_score")


#Model 3
#females have a higher hostile score than males
...
mean(data_db$Response_abs)
```

##Investigating skill in relation to gender
...
```{r with different confidence and skill dif predictions}
nd_db2 <- expand.grid(tibble(
         Follower_gender=factor(0:1) %>% rep(., times = 3),
         Leader_gender = factor(0:1) %>% rep(., times = 3),
         conf_dif_c = c(-0.75, 0, 3) %>% rep(., times = 2),
         skill_dif_roll_c = c(-0.5,0, 1) %>% rep(., times = 2),
         SubjectID = NA))
...
pred_db2 <-
  predict(model_db2, newdata = nd_db2, re_formula = ~ (1 + Follower_gender + skill_dif_roll_c:Follower_gender | SubjectID)) %>%
  as_tibble() %>%
  bind_cols(nd_db2)
...
pred_db2.1 <-
  predict(model_db2, newdata = nd_db2.1, re_formula = ~ (1 + Follower_gender + skill_dif_roll_c:Follower_gender | SubjectID)) %>%
  as_tibble() %>%
  bind_cols(nd_db2.1)
```
...
pred_db2.2 <-
  predict(model_db2, newdata = nd_db2.2, re_formula = ~ (1 + Follower_gender + skill_dif_roll_c:Follower_gender | SubjectID)) %>%
  as_tibble() %>%
  bind_cols(nd_db2.2)
```

```{r}
nd_db2.3 <- expand.grid(tibble(
         Follower_gender=factor(0:1) %>% rep(., times = 3),
         Leader_gender = factor(0:1) %>% rep(., times = 3),
         conf_dif_c = c(3) %>% rep(., times = 6),
         skill_dif_roll_c = c(0) %>% rep(., times = 6),
         SubjectID = NA))


nd_db2.3$Follower_gender <- ifelse(nd_db2.3$Follower_gender == 0, "Male", "Female")

nd_db2.3$Leader_gender <- ifelse(nd_db2.3$Leader_gender == 0, "Male", "Female")


pred_db2.3 <-
  predict(model_db2, newdata = nd_db2.3, re_formula = ~ (1 + Follower_gender + skill_dif_roll_c:Follower_gender | SubjectID)) %>%
  as_tibble() %>%
  bind_cols(nd_db2.3)
```
...
H2.1.3 <- ggplot(pred_db2.3, aes(x = Leader_gender, y = Estimate, fill = Follower_gender)) + 
  geom_violin(aes(x = Leader_gender, y = Estimate), data = pred_db2.3, width = 0.7, alpha = 0.8) + 
  scale_y_continuous(breaks = sort(c(seq(min(pred_db2.3$Estimate), max(pred_db2.3$Estimate), length.out=5), 0.5))) + 
  theme(panel.grid.minor = element_blank()) + 
  labs(x = "Leader gender", y = "Predicted propensity to surrender", title = "H2.1.3") + 
  stat_summary(fun.data=mean_sdl, geom="pointrange", color="black", position = position_dodge(width = 0.7), alpha = 0.8) +
  labs(fill = "Follower gender") + 
  scale_fill_manual(values=c("orange", "blue"))
...
```{r}
H2.1.4 <- ggplot(pred_db2, aes(x = Leader_gender, y = Estimate, fill = Follower_gender)) + 
  geom_violin(aes(x = Leader_gender, y = Estimate), data = pred_db2, width = 0.7, alpha = 0.8) + 
  scale_y_continuous(breaks = sort(c(seq(min(pred_db2$Estimate), max(pred_db2$Estimate), length.out=5), 0.5))) + 
  theme(panel.grid.minor = element_blank()) + 
  labs(x = "Leader gender", y = "Predicted propensity to surrender", title = "H2.1.3") + 
  stat_summary(fun.data=mean_sdl, geom="pointrange", color="black", position = position_dodge(width = 0.7), alpha = 0.8) +
  labs(fill = "Follower gender") + 
